...
public class MainActivity extends AppCompatActivity {
    ...
    String DATA = "微博的热搜不是说面试时不能提五险更不能提加班就是岁还单身面试被刷这可把我们吓死了现在找工作连面试这关都这么难了吗单身的我是不是得先找到对象再考虑辞职了" +
            "好友发了条朋友圈内容是这样的" +
            "同事老板我的电脑每次开机都得半个小时给我换台电脑吧" +
            "老板你就不会每天早来半个小时上班吗" +
            "还有这种操作哎跟老板比我们终究还是嫩了点" +
            "这年头谁没跟老板斗智斗勇过如果你赢了那么恭喜你老板说你可以拿单子走人了如果你输了那还是乖乖上你的班吧" +
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        EasonAdapter adapter = new EasonAdapter();
        eason.setAdapter(adapter);
        List<IndexData> list = new ArrayList<>();

        for (int i = 0; i < DATA.length(); i++) {
            list.add(new IndexData(String.valueOf(DATA.charAt(i))));
        }
